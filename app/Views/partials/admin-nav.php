<nav class="bg-white border-b border-or/20 mb-8">
    <div class="max-w-6xl mx-auto px-6 py-3 flex gap-6 overflow-x-auto text-sm">
        ...
        <a href="/admin/galerie" class="font-ui text-gris hover:text-or transition whitespace-nowrap <?= str_starts_with($_SERVER['REQUEST_URI'], '/admin/galerie') ? 'text-or font-semibold' : '' ?>">
            <i class="bi bi-images"></i> Galerie
        </a>
        <a href="/admin/reservations" class="font-ui text-gris hover:text-or transition whitespace-nowrap <?= str_starts_with($_SERVER['REQUEST_URI'], '/admin/reservations') ? 'text-or font-semibold' : '' ?>">
